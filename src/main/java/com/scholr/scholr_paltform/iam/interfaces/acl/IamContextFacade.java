package com.scholr.scholr_paltform.iam.interfaces.acl;
...
public class IamContextFacade {
   ...
   */
  public Long createUser(String username, String password, String compania, String dni, String cod_colaborador) {
    var signUpCommand = new SignUpCommand(username, password,compania,dni,cod_colaborador ,List.of(Role.getDefaultRole()));
   ...
   * @return The id of the created user.
   */
  public Long createUser(String username, String password,String compania, String dni, String cod_colaborador, List<String> roleNames) {
    var roles = roleNames != null
        ? roleNames.stream().map(Role::toRoleFromName).toList()
        : new ArrayList<Role>();
    var signUpCommand = new SignUpCommand(username, password,compania,dni,cod_colaborador , roles);
    var result = userCommandService.handle(signUpCommand);
    if (result.isEmpty())
      return 0L;
    return result.get().getId();
